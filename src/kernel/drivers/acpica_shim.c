...
ACPI_PHYSICAL_ADDRESS AcpiOsGetRootPointer(void) { return acpi_xdsp_address(); }

ACPI_STATUS AcpiOsPredefinedOverride(
    const ACPI_PREDEFINED_NAMES *init_val UNUSED, ACPI_STRING *new_val) {
  *new_val = 0;

  return AE_OK;
}

ACPI_STATUS AcpiOsTableOverride(ACPI_TABLE_HEADER *existing_table UNUSED,
...
ACPI_STATUS AcpiOsPhysicalTableOverride(
    ACPI_TABLE_HEADER *existing_table UNUSED,
    ACPI_PHYSICAL_ADDRESS *new_address, UINT32 *new_table_length UNUSED) {
  *new_address = 0;
...
ACPI_STATUS AcpiOsCreateSemaphore(UINT32 max_units UNUSED, UINT32 initial_units,
...
ACPI_STATUS AcpiOsExecute(ACPI_EXECUTE_TYPE type UNUSED,
                          ACPI_OSD_EXEC_CALLBACK function, void *context) {
  KernelThread *thread =
      thread_create((KernelThreadMain)function, context, 16, 2);
...
void AcpiOsSleep(UINT64 milliseconds) { timer_thread_sleep(milliseconds); }

void AcpiOsStall(UINT32 microseconds) { timer_thread_stall(microseconds); }
...
void AcpiOsWaitEventsComplete(void) { panic("AcpiOsWaitEventsComplete"); }
...
ACPI_STATUS AcpiOsReadPciConfiguration(ACPI_PCI_ID *pci_id, UINT32 reg,
  ...
  uint32_t word =
      pci_config_read_word(pci_id->Bus, pci_id->Device, pci_id->Function, reg);
...
ACPI_STATUS AcpiOsWritePciConfiguration(ACPI_PCI_ID *pci_id, UINT32 reg,
  ...
  pci_config_write_word(pci_id->Bus, pci_id->Device, pci_id->Function, reg,
                        word);
